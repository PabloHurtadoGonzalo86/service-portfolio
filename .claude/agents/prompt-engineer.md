...
**AI Framework:** Spring AI 2.0.0-M2 (milestone, NOT GA)
**LLM:** OpenAI GPT-4.1 via Spring AI ChatClient
**Package:** `com.example.serviceportfolio`

### Current Prompt Architecture
- **ChatClient.Builder** injected via Spring AI auto-configuration
- **Structured Output**: `entity(RepoAnalysis::class.java)` and `entity(DeveloperPortfolio::class.java)`
- **AiAnalysisService** contains all prompts for:
  - Individual repo analysis (code structure, tech stack, features → README generation)
  - Portfolio generation (multiple repos → professional portfolio with selected projects, skills, summary)

### Key Files
- `src/main/kotlin/.../services/AiAnalysisService.kt` - All AI prompts and ChatClient usage
- `src/main/kotlin/.../config/AiConfig.kt` - ChatClient bean configuration
- `src/main/kotlin/.../models/RepoAnalysis.kt` - Structured output for repo analysis
- `src/main/kotlin/.../models/DeveloperPortfolio.kt` - Structured output for portfolio
...
chatClient.prompt()
    ...
    .call()
    .entity(TargetClass::class.java)  // Structured Output
```

### Optimization Goals
- Minimize token usage (repos can be large, many files)
- Ensure consistent structured output (JSON parsing must succeed)
- Handle edge cases: empty repos, non-code repos, repos without README
