...
**DB:** PostgreSQL + Flyway + H2 (tests)
**AI:** Spring AI 2.0.0-M2 (milestone, NOT GA - APIs may differ from docs)

### Common Debugging Areas
- **Spring Security 7 + OAuth2**: SecurityFilterChain, OAuth2 login flow, `@RegisteredOAuth2AuthorizedClient`
- **DeferredResult async patterns**: 120s timeout, `aiTaskExecutor` thread pool, error propagation
- **hub4j/github-api**: GitHub App JWT tokens (1hr expiry), installation token refresh, rate limits
- **Spring AI ChatClient**: Structured Output parsing failures, OpenAI API errors, prompt issues
- **Caffeine Cache**: Cache invalidation, `@Cacheable` key conflicts
- **Bucket4j Rate Limiting**: Token bucket per IP via `RateLimitFilter`, header inspection

### Key Files for Debugging
- `src/main/kotlin/.../config/SecurityConfig.kt` - Security chain configuration
- `src/main/kotlin/.../filter/RateLimitFilter.kt` - Rate limiting filter
- `src/main/kotlin/.../client/GitHubAppTokenService.kt` - JWT + installation token management
- `src/main/kotlin/.../exceptions/GlobalExceptionHandler.kt` - Centralized error handling
- `src/test/resources/schema.sql` - H2 test schema (often source of test failures)
