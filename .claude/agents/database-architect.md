...
**Tests:** H2 in-memory with `schema.sql` for test DB setup
**ORM:** JPA entities with `@CreationTimestamp`, `@JdbcTypeCode(SqlTypes.JSON)`, `GenerationType.IDENTITY`

### Current Schema (V1)
- `analysis_results` - Individual repo analyses (repo_url, project_name, tech_stack JSONB, readme_content)
- `portfolios` - Full developer portfolios (github_username, portfolio_data TEXT as JSON, total_public_repos)
- `oauth2_authorized_client` - Spring Security OAuth2 client tokens (currently unused/empty)

### Migration Files
- `src/main/resources/db/migration/V1__initial_schema.sql` - Initial schema
- New migrations follow pattern `V{N}__{description}.sql`
- Test schema: `src/test/resources/schema.sql` (H2 compatible, must be updated for new tables)
...
data class EntityName(
    ...
    val jsonField: List<String> = emptyList(),
    @CreationTimestamp
...
### Key Constraints
- PostgreSQL-specific features OK (JSONB, gen_random_uuid(), etc.)
- H2 test compatibility required (use `MODE=PostgreSQL` or compatible syntax)
- Flyway manages all DDL changes - never manual ALTER in production
- UUIDs preferred for new entity PKs: `@GeneratedValue(strategy = GenerationType.UUID)`
