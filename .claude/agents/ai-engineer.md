...
**AI Framework:** Spring AI 2.0.0-M2 (milestone release, NOT GA - verify APIs in official docs)
**LLM Provider:** OpenAI GPT-4.1 via Spring AI auto-configuration
**Package:** `com.example.serviceportfolio`

### AI Integration Architecture
- **ChatClient.Builder** auto-configured by Spring AI, customized in `AiConfig.kt`
- **Structured Output** via `.entity(KotlinDataClass::class.java)` - Spring AI parses JSON response into Kotlin data classes
- **Two AI workflows:**
  1. **Repo Analysis**: Takes repo context (file tree, key files, metadata) → generates project description + README
  2. **Portfolio Generation**: Takes list of all user repos → selects best projects, generates professional summary, categorizes skills

### Key AI Files
- `src/main/kotlin/.../services/AiAnalysisService.kt` - ChatClient calls, prompt construction, structured output
- `src/main/kotlin/.../config/AiConfig.kt` - ChatClient bean setup
- `src/main/kotlin/.../models/RepoAnalysis.kt` - Output model for repo analysis
- `src/main/kotlin/.../models/DeveloperPortfolio.kt` - Output model for portfolio
- `src/main/kotlin/.../models/RepoContext.kt` - Input context for AI prompts

### Performance Considerations
- AI calls run on dedicated `aiTaskExecutor` thread pool (DeferredResult pattern)
- 120-second timeout on async endpoints
- Caffeine cache prevents duplicate API calls to GitHub
- DB deduplication prevents re-analyzing same repo/username
